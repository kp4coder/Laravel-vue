                    ...
                    <table id="example2" class="table table-striped table-bordered">
                        ...
                        <tbody>
                            ...
                            <tr>
                                <td>{{ $row->id }}</td>
                                <td>{{ $row->text }}</td>
                                <td style="background-color:{{ $row->value }};">{{ $row->value }}</td>
                                <td>{{ $row->created_at }}</td>
                                <td>{{ $row->updated_at }}</td>
                                <td>
                                    <button type="button" onclick="saveData('{{ $row->id }}', '{{ $row->text }}', '{{ $row->value }}')" class="btn" data-bs-toggle="modal" data-bs-target="#addModal">
                                        <i class="fadeIn animated bx bx-edit"></i>
                                    </button>
                                    <button type="button" onclick="deleteData('{{ $row->id }}', 'colors' )" class="btn delete_{{ $row->id }}">
...
<div class="modal fade" id="addModal" tabindex="-1" aria-labelledby="addModalLabel" aria-hidden="true">
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            <form action="{{ url('admin/updateColor') }}" id="formSubmit" method="POST" enctype="multipart/form-data">
                @csrf
                ...
                <div class="modal-body">
                    ...
                    <div class="row mb-3">
                        ...
                        <div class="col-sm-9">
                            <input type="text" class="form-control" id="enter_value" name="value" placeholder="" required>
                        </div>
                    </div>
